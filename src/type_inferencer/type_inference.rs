...
use crate::{
    ast::{Ast, AstNode, BinOp, Identifier, Program, SrcLoc, Symbol},
    interpreter::interpret::{find_data_declarations, InterpError},
};

use super::ast::{Constraint, ConstraintSet, Term, TypeEnv};
...
pub fn infer_types(program: &Program) -> Result<TypeEnv, InferenceError> {
    let constraint_set = generate_constraints(&program)?;
    Err(InferenceError::CyclicType())
}
